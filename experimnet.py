...
from project.fuzzer import Augment, ColorSpace, Normalize
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for img_idx in range(10000):
            bgr_numpy = cv2.imread(process.image_path[img_idx])
            rgb_numpy = color.bgr2rgb(bgr_numpy)
            fuzz1_numpy = [
                aug.tran2(image=rgb_numpy)["image"] for iter in range(batch_size)
            ]
            fuzz2_numpy = [aug.tran3(image=x)["image"] for x in fuzz1_numpy]
            fuzz3_numpy = [aug.tran4(image=x)["image"] for x in fuzz2_numpy]
            fuzz4_numpy = [aug.tran5(image=x)["image"] for x in fuzz3_numpy]
            fuzz5_numpy = [aug.tran6(image=x.copy())["image"] for x in fuzz4_numpy]
            resize_numpy = [aug.tran1(image=x)["image"] for x in fuzz5_numpy]
            norm_numpy = [nml.norm1(x) for x in resize_numpy]
