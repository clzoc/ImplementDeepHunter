...
layertopData = []
layertopData.append(np.zeros((22, 1000, 64), dtype=np.int32))
layertopData.append(np.zeros((24, 1000, 128), dtype=np.int32))
layertopData.append(np.zeros((47, 1000, 256), dtype=np.int32))
layertopData.append(np.zeros((32, 1000, 512), dtype=np.int32))
layertopData.append(np.zeros((20, 1000, 1024), dtype=np.int32))
layertopData.append(np.zeros((12, 1000, 2048), dtype=np.int32))
...
#         status = layersOut[layer][img_idx][ni]
#         if status <= 5:
#             neuronFile[layer][ni] = 1 

# load the record file
neuron = []
neuron.append(np.load("./layer_top/layerTop64.npy"))
neuron.append(np.load("./layer_top/layerTop128.npy"))
neuron.append(np.load("./layer_top/layerTop256.npy"))
neuron.append(np.load("./layer_top/layerTop512.npy"))
neuron.append(np.load("./layer_top/layerTop1024.npy"))
neuron.append(np.load("./layer_top/layerTop2048.npy"))
...
with torch.no_grad():
    for batch_idx in range(batch_blocks):
        ...
        tensor1 = torch.argsort(tensor1, dim=2).type(torch.int32)
        tensor2 = torch.argsort(tensor2, dim=2).type(torch.int32)
        tensor3 = torch.argsort(tensor3, dim=2).type(torch.int32)
        tensor4 = torch.argsort(tensor4, dim=2).type(torch.int32)
        tensor5 = torch.argsort(tensor5, dim=2).type(torch.int32)
        tensor6 = torch.argsort(tensor6, dim=2).type(torch.int32)

        recordPosition = []
        recordPosition.append(tensor1)
        recordPosition.append(tensor2)
        recordPosition.append(tensor3)
        recordPosition.append(tensor4)
        recordPosition.append(tensor5)
        recordPosition.append(tensor6)

        for i in range(6):
            forCount = recordPosition[i].cpu().numpy()
            for m in range(forCount.shape[0]):
                layertopData[i][m][batch_idx] = forCount[m][4]
...
for i in range(6):
    forCount = layertopData[i]
    for m in range(forCount.shape[0]):
        layer_index = index[i][m]
        np.save("../message_to_frontend/layerTop/{}/inf.npy".format(layer_index), layertopData[i][m]) 
