...
globalCoverageInformation = np.zeros([158, 1000], dtype=np.float32)
@nb.njit
def runsingle(file, para1):
    out = 0
    ...
    for batch_idx in range(batch_blocks):
        ...
        for i in range(6):
            ...
            for m in range(forCount.shape[0]):
                sat = runsingle(forCount[m], forCount.shape[1])
                layer_index = index[i][m] 
                globalCoverageInformation[layer_index][batch_idx] = sat / forCount.shape[1]

            satwhole += runwhole(forCount, forCount.shape[0], forCount.shape[1])
            totalwhole += forCount.shape[0] * forCount.shape[1]

        globalCoverageInformation[157][batch_idx] = satwhole / totalwhole

    for x in handle:
        x.remove()

np.save(
    "../message_to_frontend/layerTop/allCoverageInformation.npy",
    globalCoverageInformation,
)
